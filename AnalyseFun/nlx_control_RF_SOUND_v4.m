...
for j = 1:ChanIndexNum % loop trough all the selected channels
	...
	if isempty(findobj('type','figure','tag',['nlx_control_RF_SOUND ' ChanName{ChanIndex(j)}]))
        ...
        DS(j).SoundLevelSpikes= zeros(1,DS(j).StimNum,length(DS(j).Tau),length(DS(j).RFsoundVols));
        DS(j).SoundLevelSpikeCountBase = zeros(1,DS(j).StimNum,length(DS(j).RFsoundVols)); 
        DS(j).SoundLevelTrialCount= zeros(1,DS(j).StimNum,length(DS(j).RFsoundVols)); 
...
%%
		RVCOFigHandle(j) = figure( ...
			'tag',['nlx_control_RF_SOUND ' ChanName{ChanIndex(j)}], ...
			'color','k', ...
			'numbertitle','off', ...
			'name',['nlx_control_RF_SOUND ' '' ChanName{ChanIndex(j)} ''''], ...
			'menubar','none', ...
		...
		xAxis = 1:p.RFMapSize(2);
		yAxis = 1:p.RFMapSize(1);
        set(mapAx, ...
			'units','normalized', ...
			'color',[0 0 0], ...
			'fontsize',6, ...
            ...
            'ylim',DS(j).ylim,'ytick',DS(j).ytick,'yticklabel',yAxis,'ydir','reverse');
...
% 				line([-0.1 0; 0.1 0],[0 -0.1;0 0.1],'linestyle','-','linewidth',1,'color','k','tag','crosshair');
		...
		RVCOFigHandle(j) = findobj('type','figure','tag',['nlx_control_RF_SOUND '  ChanName{ChanIndex(j)}]);
...
% get current Volume Condition
volumeCondition=volumeCondition(length(volumeCondition));
...
% prepare/get Grid Histogram Data
    ...
    for j = 1:ChanIndexNum
        DS(j) = get(findobj('tag',['nlx_control_RF_SOUND ' ChanName{ChanIndex(j)}],'type','figure'),'userdata');
        ...
        for ct = t
            ...
            if length(CurrTrialStimOff{1})~=length(CurrTrialStimOff{1});error('check stim off events');end;
    % 		CurrRvcoWins = [CurrTrialStimOn{1}' [CurrTrialStimOn{1}(2:end) CurrTrialStimOff{1}]'];
            ...
            switch p.RFStimSeqDecodingMethod
                ...
                case 2 %Index of all stimuli
                    ...
                    DS(j).TrialCount(currStimIndex) = DS(j).TrialCount(currStimIndex) + 1; % trial count for each sound level
                    DS(j).SpikeCount(:,currStimIndex,:) = DS(j).SpikeCount(:,currStimIndex,:) + CurrTrialRVCO; % spike count for each sound level
                    DS(j).SpikeCountBase(:,currStimIndex,:) = DS(j).SpikeCountBase(:,currStimIndex) + CurrBaseRVCO; % need to check this!!!! 
                    
                    % Adds trial to trialcounter for each stim index
                    % (1 x stimNum x volume Cond(ie 1-4))
                    DS(j).SoundLevelTrialCount(:,currStimIndex,volumeCondition) = DS(j).SoundLevelTrialCount(:,currStimIndex,volumeCondition) + 1;
                    DS(j).SoundLevelTrialCount= DS(j).SoundLevelTrialCount(:,[1:1:size(CurrTrialRVCO,2)],:); % cuts the counter down to the number of stim along 2nd dim
                    
                    % Adds spike numbers to running total (matrix is
                    % 1 x stimNum x timewindow x volume condition
                    DS(j).SoundLevelSpikes(:,currStimIndex,:,volumeCondition) = DS(j).SoundLevelSpikes(:,currStimIndex,:,volumeCondition) + CurrTrialRVCO;
                    DS(j).SoundLevelSpikes= DS(j).SoundLevelSpikes(:,[1:1:size(CurrTrialRVCO,2)],:,:); % again cuts the matrix down to just the 23 stim length in 2nd dim
                    
                    %Adds the avarage firing across the entire window (ie
                    %500ms) to the running total matrix is 1 x stimNum x
                    %Volume cond
                    DS(j).SoundLevelSpikeCountBase(1,currStimIndex,volumeCondition) = DS(j).SoundLevelSpikeCountBase(1,currStimIndex,volumeCondition) + CurrBaseRVCO;
                    DS(j).SoundLevelSpikeCountBase = DS(j).SoundLevelSpikeCountBase(:,[1:1:size(CurrTrialRVCO,2)],:); % cuts the matrix to the number of stims, removes 0s
        ...
        set(findobj('tag',['nlx_control_RF_SOUND ' ChanName{ChanIndex(j)}],'type','figure'),'userdata',DS(j));
    end
...
%% Orginal Stuff
        ...
        %% Sound modified stuff
        DS(j).ReshapedTrialCount=[];
        
        %reshapes trial matrix to allow average spikes across all trials to
        %be calculated
        for x=1:length(DS(j).RFsoundVols)
            reshapeMat= repmat(DS(j).SoundLevelTrialCount(1,:,x),[1  1 length(DS(j).Tau)]);
            DS(j).ReshapedTrialCount= cat(4,DS(j).ReshapedTrialCount, reshapeMat);
        end
        
        DS(j).SoundLevelSpikes= DS(j).SoundLevelSpikes./DS(j).ReshapedTrialCount; % calculates average per time window
        DS(j).SoundLevelSpikeCountBase = DS(j).SoundLevelSpikeCountBase./DS(j).SoundLevelTrialCount; % calculates average per trial average
        DS(j).SoundLevelSpikes(isnan(DS(j).SoundLevelSpikes)) = 0;
        ...
        %Adapt z score stuff
         ...
         if p.RFMapZScoreFlag
            ...
            %for z=1:length(DS(j).RFsoundVols) % need to check for multiple sound levels!!!!
                DS(j).SoundLevelSpikes = (DS(j).SoundLevelSpikes-mean(DS(j).SpikeCountBase(:,NonZeroStims)))./std(DS(j).SpikeCountBase(:,NonZeroStims));
           % end
         end
         
        MaxSpikesSound(j) = max(DS(j).SoundLevelSpikes(:));
        MinSpikesSound(j) = min(DS(j).SoundLevelSpikes(:));
        if MaxSpikesSound(j)<1;MaxSpikesSound(j) = 1;end
    ...
    for j = 1:ChanIndexNum
        ...
        % plot map
        for cFigCol = 1:length(DS(j).FigColParam) % FigColParam== to the end time of time window
            for cFigRow = 1:length(DS(j).FigRowParam) % FigRow == start times of windows, equates to 1 of the four plots
%% Old PLot stuff
                ZDATAraw = zeros(p.RFMapSize); % raw 4x23 matrix
                ZDATA = zeros(p.RFMapSize.*p.RFMapImageResizeFactor); % matrix * 5
...
%% New plot stuff                
%                 ZDATArawSound = zeros(p.RFMapSize); % raw 4x23 matrix
%                 ZDATASound = zeros(p.RFMapSize.*p.RFMapImageResizeFactor); % matrix * 5
%                 for z=1:length(DS(j).RFsoundVols) % sorts plotting data into different volume levels for each time window
%                 ZDATArawSound(z,:,:) = DS(j).SoundLevelSpikes(1,:,cFigRow,z);
%                 end
                ZDATArawSound = reshape(DS(j).SoundLevelSpikes(1,:,cFigRow,1),p.RFMapSize);
                ZDATASound = imresize_old(ZDATArawSound,p.RFMapImageResizeFactor,p.RFMapImageResizeInterpolation,p.RFMapImageResizeFilterOrder);
 
                axes(findobj('tag',[num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(cFigCol))],'parent',findobj('tag',['nlx_control_RF_SOUND ' ChanName{ChanIndex(j)}],'type','figure')));


                %set(findobj('parent',gca,'type','image'),'cdata',ZDATA);
                set(findobj('parent',gca,'type','image'),'cdata',ZDATASound);

                if isempty(p.RFMapCLim)
                    %clim = [MinSpikes(j) MaxSpikes(j)];
                    clim = [MinSpikesSound(j) MaxSpikesSound(j)];
                else
                    clim = p.RFMapCLim;
                end
                ...
                if ~isempty(ctitle)
                    ...
                    set(ctitle,'string',[num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(cFigCol)) ' ' sprintf('Scale: %3.1f-%3.1f',MinSpikesSound(j),MaxSpikesSound(j))]);
                else
%                     text(textX,textY, ...
...
%                         'fontsize',10,'horizontalalignment','left','verticalalignment','bottom','color','w','tag','axestitle');
                    ...
                    text(textX,textY, ...
                        [num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(cFigCol)) ' ' sprintf('Scale: %3.1f-%3.1f',MinSpikesSound(j),MaxSpikesSound(j))], ...
                        'fontsize',10,'horizontalalignment','left','verticalalignment','bottom','color','w','tag','axestitle');
                end
    ...
    %             end
        ...
        cColMap = findobj('tag','colormap','type','axes','parent',findobj('tag',['nlx_control_RF_SOUND ' ChanName{ChanIndex(j)}],'type','figure'));
        if ~isempty(cColMap)
            axes(cColMap);
            cla;
