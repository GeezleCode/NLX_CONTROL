...
for j = 1:ChanIndexNum % loop trough all the selected channels
	if isempty(findobj('type','figure','tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}]))
		...
		RVCOFigHandle(j) = figure( ...
			'tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}], ...
			'color','k', ...
			'numbertitle','off', ...
			'name',['nlx_control_RF_SOUND_90db Channel ''' ChanName{ChanIndex(j)} ''''], ...
			'menubar','none', ...
...
%         set(cMapAx,'tag','colormap');
		...
		RVCOFigHandle(j) = findobj('type','figure','tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}]);
	end
end
...
for j = 1:ChanIndexNum
	DS(j) = get(findobj('tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}],'type','figure'),'userdata');
...
% 		CurrRvcoWins = [CurrTrialStimOn{1}' [CurrTrialStimOn{1}(2:end) CurrTrialStimOff{1}]'];
	...
	set(findobj('tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}],'type','figure'),'userdata',DS(j));
end
...
% 			
% 			axes(findobj('tag',[num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(cFigCol))],'parent',findobj('tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}],'type','figure')));
% 			
% 					
...
% 	end
%     
%     cColMap = findobj('tag','colormap','type','axes','parent',findobj('tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}],'type','figure'));
cColMap = findobj('tag','plot','type','axes','parent',findobj('tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}],'type','figure'));
    if ~isempty(cColMap)
        axes(cColMap);
        cla;
...
%set(cMapAx,'xcolor','w','ycolor','w','tag','plot');
    end
    ...
    for cFigCol = 1:length(DS(j).FigColParam)
		for cFigRow = 1:length(DS(j).FigRowParam)
            axes(findobj('tag',[num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(cFigCol))],'parent',findobj('tag',['nlx_control_RF_SOUND_90db ' ChanName{ChanIndex(j)}],'type','figure')));
             ...
             ctitle = findobj('parent',gca,'tag','axestitle');
            if ~isempty(ctitle)
                set(ctitle,'string',[num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(cFigCol)) ' ' sprintf('Scale: %3.1f-%3.1f',min(min(spikeData)),max(max(spikeData)))]);
            else
                text(textX,textY, ...
                    [num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(cFigCol)) ' ' sprintf('Scale: %3.1f-%3.1f',MinSpikes(j),MaxSpikes(j))], ...
                    'fontsize',10,'horizontalalignment','left','verticalalignment','bottom','color','b','tag','axestitle');
            end
            
            set(findobj('parent',gca,'type','image'),'cdata',spikeData(:,cFigRow)); 
    
            %plot(spikeData(:,cFigRow)); 
    
        end
    end
    DS(j).FigColParam; % this is the time end bins 
    DS(j).FigRowParam; %this is the time start bins
