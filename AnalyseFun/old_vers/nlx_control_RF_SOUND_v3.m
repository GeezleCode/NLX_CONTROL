...
for j = 1:ChanIndexNum % loop trough all the selected channels
	if isempty(findobj('type','figure','tag',['nlx_control_RF_SOUND '  ChanName{ChanIndex(j)}]))
        ...
        DS(j).climT= zeros(1, length(DS(j).RFsoundVols));
...
%% Highest db Figures
        ...
        if length(DS(j).FigColParam)==1
            mapAx = subaxes(RVCOFigHandle(j), ...
                (length(DS(j).FigRowParam)*length(DS(j).RFsoundVols)),[], ...
                [0.00005 0.01],[0.005,0.05,0.002,0.002]);
        ...
        set(mapAx, ...
            ...
            'ylim',DS(j).ylim,'ytick',DS(j).ytick,'ydir','reverse');

		% 4 columns of maps
        counter = 1;
		for i=1:length(DS(j).RFsoundVols)
			for k=1:(length(DS(j).FigRowParam))
                axes(mapAx(counter,1));
                counter = counter +1;
 				set(gca,'tag',[num2str(DS(j).FigRowParam(k)) ' ' num2str(DS(j).FigColParam(1)) ' ' num2str(DS(j).RFsoundVols(i))], 'yticklabel',DS(j).RFsoundVols(i));
 				image( ...
...
% prepare/get Grid Histogram Data
            ...
            switch p.RFStimSeqDecodingMethod
                ...
                case 2 %Index of all stimuli
                    ...
                    if currStimIndex(end)==0; % Added hack to stop code crashing, still need to investigate
                       currStimIndex= currStimIndex(1:(end-1));
                       disp('Corrected Stim Params');
                    end
                    % get the SPIKES for last trial
    ...
    switch volumeCondition(length(volumeCondition))
        ...
        case 1
            ...
            for cFigCol = 1
                for cFigRow = 1:length(DS(j).FigRowParam)
                    ...
                    cLumIndex = (DS(j).Lum==DS(j).FigColParam(1));
        ...
        % 			ZDATA = imresize(ZDATAraw,p.RFMapImageResizeFactor,p.RFMapImageResizeInterpolation,p.RFMapImageResizeFilterOrder);
                    ...
                    axes(findobj('tag',[num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(1)) ' ' num2str(DS(j).RFsoundVols(cFigCol))],'parent',findobj('tag',['nlx_control_RF_SOUND ' ChanName{ChanIndex(j)}],'type','figure')));
                    ...
                    if ~isempty(ctitle)
                        set(ctitle,'string',[num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(1)) ' ' num2str(DS(j).RFsoundVols(cFigCol)) ' ' sprintf('Scale: %3.1f-%3.1f',MinSpikes(j),MaxSpikes(j))]);
                    else
                        text(textX,textY, ...
                            [num2str(DS(j).FigRowParam(cFigRow)) ' ' num2str(DS(j).FigColParam(1)) ' ' num2str(DS(j).RFsoundVols(cFigCol)) ' ' sprintf('Scale: %3.1f-%3.1f',MinSpikes(j),MaxSpikes(j))], ...
