function s = nlx_control_settings_RF2SOUND
...
%---------- RF map parameter ----------------------------------------
%**********************************************************************
%s.RFMapSize = [1 23];% rows cols of map
s.RFMapSize = [4 21];% rows cols of map
s.RFsoundVols = [50 60 70 80];
s.RFDotSpacing = 0.5;%e.g. RFRC150 --> 1.5
s.RFMapRefPos = [-1 -3]; %% RF Position in Par-File
s.RVCOTau = [180];
s.RVCOTauBase = 0;
s.RVCOWin = [-150 0];
s.RFDotLum(1:prod(s.RFMapSize)) =  500;
...
s.SendParamN = s.RFMapSize(2)+s.RFStimSeqIndex_SEQStart-1;
