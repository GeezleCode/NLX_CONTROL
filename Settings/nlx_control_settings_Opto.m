function s = nlx_control_settings_Opto
...
s.EventColor = repmat([0 1 1],length(s.EventName),1); % light blue
% s.EventColor(4,:,:) = [1 1 1]; % colours stim on events differently
...
s.CndAlignEvent = s.EventCode(strmatch('NLX_SUBJECT_END',s.EventName));
...
%  s.CurrTrialTimeTicks = [-2000:500:2000];
 s.CurrTrialTimeTicks = [0:100:3000];
...
s.RasterTimeLim = [0 2000];
% s.RasterTimeTicks = [-2000:100:2000];
s.RasterTimeTicks = [0:200:2000];
