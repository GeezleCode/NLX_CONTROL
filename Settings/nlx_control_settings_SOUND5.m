function s = nlx_control_settings_SOUND5
...
s.MandatoryEvents = [255 4 32 64 128];
...
%---------- parameters for online the default online analysis (i.e. trial-raster, trial-stats, histogram, raster) ----------------------------------------
s.CndPlotGrid   = [1];
s.StimCodeGrid  = [1];
s.CurrTrialTimeTicks = [0:500:10000];
s.CurrTrialAlignEventName = 'NLX_TRIAL_START';
s.RasterTrialNum = 10;
s.RasterDotSize = 1;
s.RasterEventSize = 3.0;
s.RasterTimeLim = [0 10000];
s.RasterTimeTicks = [0:1000:10000];
s.RasterAlignEventName = 'NLX_TRIAL_START';
s.RasterAlignEventCode = s.EventCode(strmatch('NLX_TRIAL_START',s.EventName));
...
% ---- extra online parameter ----
a.CndParamLabel = {'frequency' 'volume'}; % labels the rows of s.CndParam
a.CndParam{1}   = [1000 1189 1414 1682 2000 2378 2828 3364 4000 4757 5657 6727 8000 9514 11314 13454 16000 19027 22627 26909 32000];
a.CndParam{2}   = [1 2 3 4];
a.CndParamNum   = cellfun('length',a.CndParam);
a.CndName       = strtrim(cellstr(num2str([1:s.Cndnum]')))';
% a.colormap   = jet;
% a.colormap   = usercolormap(rgb('dark blue'),rgb('blue'),rgb('cyan'),rgb('black'),rgb('yellow'),rgb('red'),rgb('dark red'));
% a.colormap   = usercolormap(rgb('blue'),rgb('cyan'),rgb('very dark green'),rgb('yellow'),rgb('red'));
