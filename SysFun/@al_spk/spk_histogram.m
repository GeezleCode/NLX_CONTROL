function [values,binedges,h] = spk_histogram(s,timewin,binwidth,HistMode,PlotType,varargin)
...
for i=s.currentchan
    if numspk(i)>0
        values(:,find(s.currentchan==i)) = histc(cat(2,s.spk{i,s.currenttrials}),binedges)'.*normfactor;
    else
