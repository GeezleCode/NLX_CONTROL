...
function [succeeded, cheetahObjects, cheetahTypes] = NlxGetCheetahObjectsAndTypes()  
	...
	if succeeded == 1
		[succeeded, cheetahObjects, cheetahTypes] = calllib('MatlabNetComClient', 'GetCheetahObjectsAndTypes', cheetahObjectsPointer, cheetahTypesPointer);
    end;
    
    if succeeded == 1
		for index = 1:MAX_OBJECTS
			if strcmp(STRING_PLACEHOLDER, cheetahObjects(index)) == 1
				cheetahObjects = cheetahObjects(1:index-1);
				cheetahTypes = cheetahTypes(1:index-1);
				break;
			end;     
		end;
    end;
